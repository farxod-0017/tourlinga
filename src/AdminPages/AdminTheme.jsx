...
import users from "../Images/termin.svg"
import savol from "../Images/savol.svg"
...
export default function AdminThemes() {
    ...
    async function updateDirec(e) {
        ...
        if (timeDiff < 900) {
            try {
                setIsLoading(true);
                let fetchData = await fetch(`${mURL}/main/mavzular/${updingId}/`, {
                    method: 'PUT',
                    headers: {
                        'Authorization': `Bearer ${takeOriginalValue('access_token')}`
                    },
    ...
    async function deleteDirecOk(e) {
        ...
        if (timeDiff < 900) {
            try {
                setIsLoading(true);
                let fetchData = await fetch(`${mURL}/main/mavzular/${deltingId}/`, {
                    method: 'DELETE',
                    headers: {
                        'Authorization': `Bearer ${takeOriginalValue('access_token')}`
                    },
        ...
        <section className="adm_unv">
            ...
            <div ref={del_modal} className="adm_news_modal">
                <div className="adm_del_modal_window">
                    <img src={undov} alt="" />
                    <h4>Mavzuni o'chirish</h4>
                    <p>Haqiqattan ham bu mavzuni({delTitle}) oʻchirib tashlamoqchimisiz? Bu amalni ortga qaytarib bo‘lmaydi.</p>
            ...
            <div ref={pp_modal} className="adm_news_modal">
                <div className="adm_del_modal_window adm_pp_modal_window">
                    <h4>Mavzu {modalMood ? "qo‘shish" : "yangilash"} </h4>
                    <p>Mavzu nomi</p>
                    <input value={direction_name} onChange={(e) => setDirectionName(e.target.value)} placeholder="Mavzu nomini kiriting" type="text" />
                    <div className="del_btn_wrap">
                        {modalMood ? (
                            <span>
