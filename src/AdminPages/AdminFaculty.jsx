...
export default function AdminFaculty() {
    ...
    let [unver_id, setUnver_id ] = useState("");
    ...
    async function updateDirec(e) {
        ...
        const readyD = new FormData();
        readyD.append('name', direction_name);
        readyD.append('university', unver_id)

        if (timeDiff < 900) {
            try {
                setIsLoading(true);
                let fetchData = await fetch(`${mURL}/universities/faculties/${updingId}/`, {
                    method: 'PUT',
                    headers: {
                        'Authorization': `Bearer ${takeOriginalValue('access_token')}`
                    },
    ...
    async function deleteDirecOk(e) {
        ...
        if (timeDiff < 900) {
            try {
                setIsLoading(true);
                let fetchData = await fetch(`${mURL}/universities/faculties/${deltingId}/`, {
                    method: 'DELETE',
                    headers: {
                        'Authorization': `Bearer ${takeOriginalValue('access_token')}`
                    },
    ...
    const getUnv = useCallback(async () => {
        const current_time = new Date();
        const stored_time = takeOriginalValue('stored_time');
        const timeDiff = (current_time - new Date(stored_time)) / 60000;
        if (timeDiff < 900) {
        ...
        <section className="adm_unv">
            ...
            <div ref={del_modal} className="adm_news_modal">
                <div className="adm_del_modal_window">
                    <img src={undov} alt="" />
                    <h4>Fakultetni o'chirish</h4>
                    <p>Haqiqattan ham bu Fakultetni({delTitle}) oʻchirib tashlamoqchimisiz? Bu amalni ortga qaytarib bo‘lmaydi.</p>
            ...
            <div ref={pp_modal} className="adm_news_modal">
                <div className="adm_del_modal_window adm_pp_modal_window">
                    <h4>Fakultet {modalMood ? "qo‘shish" : "yangilash"}</h4>
                    <p>Fakultet nomi</p>
                    <input value={direction_name} onChange={(e) => setDirectionName(e.target.value)} placeholder="Fakultet nomini kiriting" type="text" />
                        <div>
                            <p>Universteti</p>
                            <select value={unver_id} onChange={(e)=>setUnver_id(e.target.value)}>
                                {unv?.map((item) => {
                                    return (
                            ...
                            <div className="adm_unv_box_top">
                                ...
                                <div className="unv_pp_dots">
                                    ...
                                    <div className={`unv_pp_window`} id={item.visible ? `open_unv_pp_window` : ""}>
                                        <span onClick={(e) => openUpdateModal(e, { title: item.name, id: item.id, unver:item.university })}>
                                            <img src={edit} alt="" />
                                            <h6>Taxrirlash</h6>
                                        </span>
